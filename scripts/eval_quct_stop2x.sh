...
TS="$(date +%m%d_%H%M%S)"
SAVE_PATH="eval_log/quct_stop2x_t0p1_12w_${TS}"
...
for IDX in $(seq 0 11); do
    ...
    CUDA_VISIBLE_DEVICES=${gpus[$IDX]} "$PYTHON" src/eval/eval_vllm.py \
        ...
        > "eval_log/runtime_logs/stop2x_split_${IDX}_${TS}.log" 2>&1 &
done

wait

"$PYTHON" src/eval/analyze_results.py --path "$SAVE_PATH" \
    > "eval_log/runtime_logs/stop2x_analyze_${TS}.log" 2>&1
