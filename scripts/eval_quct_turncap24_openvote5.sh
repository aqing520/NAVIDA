...
TS="$(date +%m%d_%H%M%S)"
SAVE_PATH="eval_log/quct_turncap2_openvote5_k5_t0p1_24w_${TS}"
...
for IDX in $(seq 0 23); do
    ...
    CUDA_VISIBLE_DEVICES=${gpus[$IDX]} "$PYTHON" src/eval/eval_vllm.py \
        ...
        > "eval_log/runtime_logs/openvote5_24_split_${IDX}_${TS}.log" 2>&1 &
done

wait

"$PYTHON" src/eval/analyze_results.py --path "$SAVE_PATH" \
    > "eval_log/runtime_logs/openvote5_24_analyze_${TS}.log" 2>&1
